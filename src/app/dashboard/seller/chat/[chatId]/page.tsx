...
export default function SellerChat() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            if (chat) {
                ...
                const buyerUser = await store.getUser(chat.buyerId);
                setBuyer(buyerUser || null);
    ...
    if (isLoading) return <div className="min-h-screen flex items-center justify-center"><Loader2 className="animate-spin text-indigo-600" size={32} /></div>;
    if (!currentChat || !buyer) return <div className="p-8 text-center">Chat not found</div>;
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-3">
                        <div className="h-10 w-10 rounded-full bg-emerald-100 flex items-center justify-center overflow-hidden border border-emerald-200">
                            {buyer.profilePic ? (
                                <img src={buyer.profilePic} alt={buyer.name} className="h-full w-full object-cover" />
                            ) : (
                                <span className="font-bold text-emerald-700">{buyer.name[0]}</span>
                            )}
                        </div>
                        <div>
                            <h2 className="font-bold text-slate-900 text-sm">{buyer.name}</h2>
                            {listing && (
                ...
                {messages.map((msg) => {
                    ...
                    return (
                        <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                            ...
                            >
                                <p>{msg.content}</p>
                                <p className={`text-[10px] mt-1 ${isMe ? 'text-indigo-100' : 'text-slate-400'}`}>
            ...
            <div className="bg-white border-t border-slate-200 p-4 sticky bottom-0">
                <form onSubmit={handleSendMessage} className="flex gap-2 max-w-4xl mx-auto">
                    <input
                        ...
                        className="flex-1 px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-full focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all"
                    />
                    <button
                        type="submit"
